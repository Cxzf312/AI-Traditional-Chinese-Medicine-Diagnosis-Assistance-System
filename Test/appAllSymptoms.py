...
# 中介點: 靜態變數(傳遞gen_frames() -> result())
# 紀錄 healthy or unhealthy 到網頁
...
# 中介點: return舌象名稱的靜態變數
def returnTongueManifestation(init=None):
    if init is not None:
        returnTongueManifestation.ee = init
    return returnTongueManifestation.ee

# 中介點: YDTS辨識信心指數
def returnYDTSconfidence(init=None):
    if init is not None:
        returnYDTSconfidence.ee = init
    return returnYDTSconfidence.ee

def gen_frames():
    global current_stage
    tongue_model = YOLO("yolo-Weights/p1_tongue300_best.pt", "v8") 
    classNames = ["tongue"]
...
# -------------------------------------------------------- 第三階段: all symptoms -----------------------------------------------------------
    ...
    #                     returnYDTSconfidence(allSymptoms_confidence) #傳遞YDTS辨識信心指數
...
def result():
    img_url = ""
    healthyState_str = "健康" #預設健康狀態為 "健康"
    tongueManifestation_str = "健康舌象" #預設舌象名稱為 "健康舌象"
    identificationConfidenceIndex = 1 #預設辨識信心指數 
    ...
    if returnHealthyState() == "healthy":
        ...
        if returnTongueManifestation() == "YDTS":
            ...
            identificationConfidenceIndex = returnYDTSconfidence()*100
        ...
        pass
    return render_template('result_allSymptoms.html', img_url=img_url, healthyState_str=healthyState_str, tongueManifestation_str=tongueManifestation_str, identificationConfidenceIndex=identificationConfidenceIndex)
